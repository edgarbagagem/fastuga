...
function isOrderEmpty() {
  ...
  if (isEmpty.value) {
    toast.error(
      "Your order is empty. Add items to your order and complete the checkout"
    );
  } else {
...
const createPayment = () => {
  ...
  if (userStore.userId != -1) {
    if (
      paymentOption.value != "mbway" &&
      paymentOption.value != "paypal" &&
      paymentOption.value != "visa" &&
      paymentOption.value != ""
    ) {
  ...
  if (!userStore.user) {
    ...
    order.value.date =
      order.value.date.getFullYear() +
      "/" +
      (order.value.date.getMonth() + 1) +
      "/" +
      order.value.date.getDate();
    ...
    order.value.total_price = parseFloat(orderStore.totalPrice);
    order.value.total_paid = parseFloat(
      (orderStore.totalPrice - (pointsToUse.value / 10) * 5).toFixed(2)
    );
    ...
    order.value.date =
      ...
      order.value.date.getDate();
    order.value.ticket_number = ticketNumberStore.ticket;
    order.value.points_gained = userStore.calculatePoints(
      order.value.total_paid
    )
      ? userStore.calculatePoints(order.value.total_paid)
      : 0;
    //order.value.paymentOption = paymentOption.value
    if (pointsToUse.value != 0) {
      order.value.points_used_to_pay = pointsToUse.value;
      order.value.total_paid_with_points = parseFloat(
        ((pointsToUse.value / 10) * 5).toFixed(2)
      );
...
<template>
  <table class="table" v-if="orderStore.totalProducts">
    ...
    <tbody>
      <tr
        v-for="produto in orderStore.productsInOrder"
        :key="produto.id"
        class="align-middle"
      >
        ...
        <td class="align-middle">
          <img
            :src="orderStore.photoFullUrl(produto)"
  ...
  <div
    ...
    v-show="!checkout"
    v-if="orderStore.totalProducts"
  >
    <div id="d-flex justify-content-end">
      <a id="qty"
        >Quantity:
        {{ orderStore.totalProducts ? orderStore.totalProducts : "0" }}</a
      >
    </div>
    <div id="d-flex justify-content-end">
      <a id="price"
        >Price: {{ orderStore.totalPrice ? orderStore.totalPrice : "0" }} €</a
      >
    </div>
  ...
  <div class="vertical-center" id="checkout" v-show="checkout">
    ...
    <div class="vertical-right">
      <a id="qty"
        ><b
          >Quantity:
          {{ orderStore.totalProducts ? orderStore.totalProducts : "0" }}</b
        ></a
      >
    </div>
    <div class="vertical-right">
      <a id="price"
        ><b
          >Price: {{ orderStore.totalPrice ? orderStore.totalPrice : "0" }} €</b
        ></a
      >
    </div>
    <div v-if="userStore.user">
      <div class="vertical-right">
        <a id="price"
          ><b
            >My Points:
            {{ userStore.user.points ? userStore.user.points : "0" }}</b
          ></a
        >
      </div>
      <div class="vertical-right">
      ...
      <div class="vertical-right">
        <a id="price"
          ><b
            ...
            {{
              pointsToUse != 0
                ? (orderStore.totalPrice - (pointsToUse / 10) * 5).toFixed(2)
    ...
    <div class="vertical-right" id="selectPaymentType">
      ...
      <select class="form-select" name="tipoPagamento" v-model="paymentOption">
        <option value="Default" v-if="userStore.userId != -1">
          Default payment type: {{ userStore.user.default_payment_type }}
        </option>
        <option value="mbway"><b>MBWAY</b></option>
        <option value="paypal">PAYPAL</option>
        <option value="visa">VISA</option>
        <!-- Continuar aqui. Se for um dos tipos escolhidos pedir as cenas como faço c o anonimo
      ...
      <div v-if="paymentOption == 'mbway'" style="padding-top: 10px">
        <input
          ...
          class="form-control"
          id="inputName"
          placeholder="MBWay Phone Number"
      ...
      <div v-if="paymentOption == 'visa'" style="padding-top: 10px">
        <input
          ...
          class="form-control"
          id="inputName"
          placeholder="VISA Card ID (16 digits)"
          v-model="paymentReference"
          required
        />
      </div>
  ...
  <div id="noProductsText">
    <a v-if="!orderStore.totalProducts"
      >To see your order, add products first.</a
    >
  </div>
</template>
<style>
