...
const deleteClick = (order) => {
  ...
  if (error.response.status == 422) {
    toast.error(
      "Order not canceled. Payment was not refunded due to validation errors!"
    );
  } else {
    toast.error(
      "Order not canceled. Payment was not refunded due to unknown server error!"
    );
  }
};
